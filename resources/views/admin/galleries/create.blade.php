...
<div class="max-w-xl bg-white p-6 rounded-xl shadow-sm border border-gray-100">
    <h1 class="text-xl font-bold text-gray-800 mb-6">Upload Foto Galeri Baru</h1>
    ...
    <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Pilih File Foto</label>
            <input type="file" id="imageInput" name="image" accept="image/*" required class="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none">
        ...
        <div>
            ...
            <input type="text" name="category" value="{{ old('category') }}" placeholder="Contoh: Perayaan Natal, Futsal, Kegiatan Paroki" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
        </div>
