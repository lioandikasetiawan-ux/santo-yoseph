...
@section('title', $announcement->title . ' - Paroki')

@section('content')
<div class="max-w-4xl mx-auto px-4 py-10">
    <a href="{{ route('public.announcements') }}" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 mb-6">
        &larr; Kembali ke Daftar Pengumuman
    </a>
    
    <article class="bg-white p-8 rounded-xl shadow-sm border border-gray-100">
        <div class="border-b pb-4 mb-6">
            <span class="text-xs font-medium text-gray-400">Dipublikasikan pada {{ $announcement->created_at->translatedFormat('d F Y') }}</span>
            <h1 class="text-3xl font-bold text-gray-900 mt-2 leading-tight">{{ $announcement->title }}</h1>
        </div>
