...
@section('title', 'Agenda Kegiatan - Paroki')

@section('content')
<div class="bg-blue-900 text-white py-12 mb-8">
    <div class="max-w-7xl mx-auto px-4 text-center">
        <h1 class="text-3xl font-extrabold mb-2">Agenda & Kegiatan Paroki</h1>
        <p class="text-blue-100 text-sm">Informasi jadwal kegiatan dan acara mendatang di lingkungan paroki.</p>
    </div>
</div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col justify-between hover:shadow-md transition">
            <div>
                @if($item->banner_image)
                    <img src="{{ asset('storage/' . $item->banner_image) }}" class="w-full h-48 object-cover">
                @else
                ...
                <div class="p-5">
                    <span class="text-xs font-semibold bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full">
                        {{ \Carbon\Carbon::parse($item->event_date)->translatedFormat('d M Y - H:i') }} WIB
                    </span>
                    <h3 class="font-bold text-lg text-gray-800 mt-3 mb-1">{{ $item->title }}</h3>
                    <p class="text-xs text-gray-500 mb-2">📍 {{ $item->location }}</p>
                    <p class="text-gray-600 text-sm line-clamp-2">{{ Str::limit(strip_tags($item->description), 100) }}</p>
                </div>
            </div>

            <div class="px-5 py-3 bg-gray-50 border-t border-gray-100 text-right">
                <a href="{{ route('public.events.detail', $item->slug) }}" class="text-blue-600 text-xs font-semibold hover:underline inline-flex items-center space-x-1">
